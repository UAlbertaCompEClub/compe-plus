...
it('rejects non-uuid resume review', async () => {
    ...
    await getMyDocuments(req as Request<Params>, res as Response, next);
...
it('correctly downloads the document', async () => {
    mockDocumentRepository.getAssociatedToUser.mockResolvedValueOnce([testConstants.document1]);
